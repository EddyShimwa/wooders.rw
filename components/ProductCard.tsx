    ...
    >
      <div className="relative aspect-[4/5] w-full overflow-hidden rounded-xl bg-muted/20">
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-700 ease-out scale-90 group-hover:scale-100">
           <div className="flex items-center gap-2 bg-white/10 backdrop-blur-2xl text-white py-2.5 px-5 rounded-full text-[10px] font-normal tracking-[0.2em] uppercase border border-white/20 shadow-2xl">
      ...
      <div className="flex flex-col gap-1.5 px-1">
        <div className="flex items-center justify-between">
          <span className="text-[8px] font-normal tracking-[0.25em] uppercase text-wood-light/80">{product.category}</span>
          <span className="text-[12px] font-normal text-wood-dark">
            <span className="text-[8px] text-wood-medium/50 mr-0.5">RWF</span>
            {product.price?.toLocaleString()}
          </span>
        </div>
        <div>
          <h3 className="text-sm md:text-base font-normal tracking-tight text-wood-dark group-hover:text-wood-light transition-colors duration-500 leading-tight">{product.name}</h3>
          <p className="text-[11px] text-wood-dark/50 mt-1 line-clamp-2 font-normal leading-relaxed">{product.description}</p>
        </div>
      </div>
    </div>
  );
